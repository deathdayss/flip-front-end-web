...
const LastestRecommand = () => {
    ...
    useEffect(
        () => {
            // const timer = null;
            if(val_showOption){set_listContent( ()=>getRecommService({zone:"test", num:5}), 10000);}
            else              {set_listContent( ()=>getLatestService({zone:"test", num:5}), 10000);}
            // return () => {window.clearTimeout(timer);}
            console.log(val_listContent)
    },[val_showOption]);
